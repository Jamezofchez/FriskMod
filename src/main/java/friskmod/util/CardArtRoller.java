...
import friskmod.util.ImageHelper;
...
public class CardArtRoller {
    private static final Texture attackMask = TexLoader.getTexture(FriskMod.imagePath("masks/AttackMask.png"));
    private static final Texture skillMask = TexLoader.getTexture(FriskMod.imagePath("masks/SkillMask.png"));
    private static final Texture powerMask = TexLoader.getTexture(FriskMod.imagePath("masks/PowerMask.png"));
        ...
        c.portrait = doneCards.computeIfAbsent(c.cardID, key -> {
            ReskinInfo r = infos.computeIfAbsent(key, key2 -> {
                ...
                if (c.reskinInfo(q) != null) {
                    return c.reskinInfo(q); //auto-changed
                } else {
